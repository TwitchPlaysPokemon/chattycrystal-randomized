BattleAnimations::
	indirect_table 2, 0
	indirect_entries ECHOED_VOICE, MoveAnimations1
	indirect_entries CHATTY_TRANSFORM - 1, MoveAnimations2
	indirect_entries CHATTY_TRANSFORM, MoveAnimationsChattyTransform
	indirect_entries CHATTY_HP - 1, MoveAnimations3
	indirect_entries CHATTY_HP, MoveAnimationsChattyHP
	indirect_entries SPECIAL_ANIM_OFFSET - NUM_SPECIAL_ANIMS - 1
	indirect_entries SPECIAL_ANIM_OFFSET - 1, SpecialAnimations
	indirect_table_end
